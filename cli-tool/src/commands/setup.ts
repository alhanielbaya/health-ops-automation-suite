...
export async function setupCommand(options: SetupOptions): Promise<void> {
  ...
  console.log(
    chalk.gray("Configuring workstation for healthcare environment\n"),
  );
  ...
  try {
    ...
    const deptConfig =
      DEPARTMENT_CONFIGS[userInfo.department] || DEPARTMENT_CONFIGS["IT"];
    ...
    const softwareConfig = await collectSoftwareRequirements(
      userInfo.department,
      deptConfig,
    );
    ...
    const confirmed = await confirmHealthcareSetup(
      userInfo,
      assetInfo,
      softwareConfig,
      vpnConfig,
    ...
    if (!options.dryRun) {
      ...
      try {
        ...
        if (existingUser) {
          spinner1.info(
            chalk.yellow(
              `User ${userInfo.employeeId} exists, using existing record`,
            ),
          );
          ...
          spinner1.succeed(
            chalk.green(
              `✓ User created: Dr. ${userInfo.lastName} (ID: ${userId})`,
            ),
          );
        }
      } catch (error) {
    ...
    if (!options.dryRun && userId) {
      ...
      try {
        ...
        spinner2.succeed(
          chalk.green(
            `✓ Workstation assigned: ${assetInfo.assetTag} (ID: ${assetId})`,
          ),
        );

        // Configure network for specific building
        if (!options.skipNetwork) {
          const spinnerNet = ora(
            `Configuring network for ${deptConfig.building}...`,
          ).start();
          await new Promise((resolve) => setTimeout(resolve, 500));
          spinnerNet.succeed(
            chalk.green(
              `✓ Network configured: ${assetInfo.ipAddress} (${deptConfig.building})`,
            ),
          );
        }
      } catch (error) {
      ...
      console.log(
        chalk.yellow("[DRY RUN] Would assign workstation:"),
        assetInfo.assetTag,
      );
    }
    ...
    if (!options.skipSoftware && !options.dryRun && assetId) {
      ...
      for (const app of standardSoftware) {
        ...
        try {
          ...
          if (app.fdaApproved) {
            spinner.succeed(
              chalk.green(`✓ ${app.name} installed (FDA-validated)`),
            );
          } else {
      ...
      console.log(
        chalk.yellow("[DRY RUN] Would install healthcare software packages"),
      );
    }
    ...
    if (!options.dryRun && vpnConfig.enabled && assetId) {
      ...
      try {
        ...
        spinnerVPN.succeed(
          chalk.green(`✓ VPN configured: ${vpnConfig.profile} profile`),
        );
        console.log(
          chalk.gray(`   Remote sessions enabled for ${userInfo.department}`),
        );
      } catch (error) {
    ...
    console.log(
      chalk.white(`👤 User: Dr. ${userInfo.firstName} ${userInfo.lastName}`),
    );
    ...
    console.log(
      chalk.white(
        `   Network: ${assetInfo.ipAddress} (${deptConfig.building})`,
      ),
    );
    console.log(chalk.gray("─".repeat(60)));
    console.log(
      chalk.white(
        `🔒 VPN Access: ${vpnConfig.enabled ? "Enabled ✓" : "Not Required"}`,
      ),
    );
    console.log(
      chalk.white(
        `💿 Software: ${deptConfig.software.length + softwareConfig.customSoftware.length} packages installed`,
      ),
    );

    if (deptConfig.software.some((s: any) => s.fdaApproved)) {
      console.log(
        chalk.white(`   ✓ FDA-validated medical software configured`),
      );
    }
...
async function runSarahJohnsonScenario(options: SetupOptions): Promise<void> {
  ...
  try {
    ...
    console.log(
      chalk.gray(`  User: ${userInfo.firstName} ${userInfo.lastName}`),
    );
    ...
    console.log(
      chalk.gray(`  Network: ${assetInfo.ipAddress} (${deptConfig.building})`),
    );
    ...
    if (!options.dryRun) {
      ...
      assetSpinner.succeed(
        chalk.green(`✓ Workstation assigned (ID: ${assetId})`),
      );
      ...
      netSpinner.succeed(
        chalk.green(`✓ Network configured: ${assetInfo.ipAddress}`),
      );
      ...
      await db.updateVPNConfig(assetInfo.assetTag, {
        enabled: true,
        profile: "Healthcare Standard",
      });
      vpnSpinner.succeed(
        chalk.green("✓ VPN enabled for remote behavioral health sessions"),
      );
      ...
      console.log(chalk.gray("─".repeat(60)));
      console.log(
        chalk.white(`👤 Dr. ${userInfo.lastName} (${userInfo.employeeId})`),
      );
      console.log(chalk.white(`   ${userInfo.jobTitle}`));
      console.log(chalk.white(`   ${userInfo.email}`));
      console.log(chalk.gray("─".repeat(60)));
      console.log(
        chalk.white(`💻 ${assetInfo.assetTag} @ ${assetInfo.location}`),
      );
  ...
  const questions = [
    {
      ...
      validate: (input: string) =>
        input.length > 0 || "Employee ID is required",
    },
    {
      type: "input",
      name: "firstName",
    ...
    },
    {
      ...
      choices: Object.keys(DEPARTMENT_CONFIGS).map((dept, index) => ({
        name: `${index + 1}. ${dept}`,
        value: dept,
      })),
      pageSize: 10,
...
async function collectAssetInfo(
  assetTag?: string,
  deptConfig?: any,
): Promise<any> {
  const defaultNetwork = deptConfig
    ? BUILDING_NETWORKS[deptConfig.building]
    : null;
  const nextIP = defaultNetwork
    ? `${defaultNetwork.subnet}.100`
    : "192.168.1.100";
  ...
  const questions = [
    ...
    },
    {
      ...
      message: "Hostname:",
      default: (answers: any) =>
        answers.assetTag?.toLowerCase().replace(/-/g, "-") || "ws-new",
    ...
    },
    {
      ...
      default: deptConfig?.building
        ? `${deptConfig.building} - Floor 2`
        : "Building A",
  ...
  return {
    ...
    department: deptConfig
      ? Object.keys(DEPARTMENT_CONFIGS).find(
          (key) => DEPARTMENT_CONFIGS[key] === deptConfig,
        )
      : "IT",
    status: "active",
...
async function collectSoftwareRequirements(
  department: string,
  deptConfig: any,
): Promise<any> {
  ...
  const { profile } = await inquirer.prompt([
    {
      ...
      name: "profile",
      message: "VPN Profile:",
      choices: ["Healthcare Standard", "Physician Extended", "Admin Limited"],
